...
This project follows the generic `/release-check` checklist plus native-addon steps it does not cover. The version-bump commit also resets `artifactHashes` to `{}` (`npm pkg set artifactHashes='{}' --json`): the hashes belong to the publish, not the tag, and an empty bag makes no false claim — a stale bag from the previous release would, and an absent key would disable verification altogether (`install-from-cache` skips the check when there is no bag). Push the version tag (`X.Y.Z`, no `v` prefix) first: `.github/workflows/build.yml` triggers on the tag — it auto-creates the GitHub release and builds + uploads a prebuilt `re2.node` for every cell of its build matrix (each OS/arch × each supported Node version) as a brotli `.br` asset via `npm run save-to-github`, each with a build-provenance attestation. Run `npm publish` only after **all** assets are uploaded: the `prepublishOnly` hook (`hash-github-cache --write`) hashes the release's assets and stamps the `artifactHashes` map into `package.json`, which `install-from-cache` verifies downloads against. Commit the stamped `package.json` after the release. The tag therefore never carries its own release's hashes: a git install of a tag (`npm install uhop/node-re2#X.Y.Z`) downloads the binary, fails verification, and builds from source by design — the npm tarball is the verified artifact. Automating the round-trip is deferred to the N-API build overhaul.
