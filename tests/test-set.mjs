...
test('test set performance', t => {
  ...
  const setResult = measure(str => (set.test(str) ? 1 : 0));
  const re2Result = measure(str => {
    for (let i = 0; i < re2List.length; ++i) {
      if (re2List[i].test(str)) return 1;
    }
    return 0;
  });
  const jsResult = measure(str => {
  ...
  t.equal(setResult.matches, re2Result.matches);
  t.equal(setResult.matches, jsResult.matches);
  t.ok(setResult.time < re2Result.time);
});
